...
const TodoSlice = createSlice({
  ...
  reducers: {
    getContact: (state, action) => {
      state.contacts = state.contacts.find((item) => item.id == action.payload);
    },
    ...
    updateContact: (state, action) => {
      state.contacts = state.contacts((item) => item.id === action.payload ? action.payload : item;
      );
    },
  },
});
const ContactReducer = TodoSlice.reducer;
export const { addContact, deleteContact, getContact, updateContact } =
  TodoSlice.actions;
